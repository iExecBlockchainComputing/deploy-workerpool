...
## Corriger les env pour joindre le CHAIN_ADAPTER
for index,var in enumerate(core_compose['services']['core']['environment']):
  if re.match("^IEXEC_CORE_CHAIN_ADAPTER_PORT=", var):
    newvar = "IEXEC_CORE_CHAIN_ADAPTER_PORT=%s" % core_compose['services']['blockchain-adapter']['ports'][0].split(':')[0]
...
to_remove = []
for index,var in enumerate(worker_compose['services']['worker']['environment']):
  ## Fix env to let the worker connect to the core
  ...
  if re.match("^IEXEC_DEVELOPER_LOGGER_ENABLED=", var):
    ...
    worker_compose['services']['worker']['environment'][index] = newvar
    print("changed %s to %s" % (var,newvar))
  ## Move variables related to SGX-Scontain (deleted in minimal version)
...
for var in to_remove:
  worker_compose['services']['worker']['environment'].remove(var)
  print("no more %s" % var)


## save files
with open('./worker/docker-compose.yml', 'w') as file:
  yaml.dump(worker_compose, file)